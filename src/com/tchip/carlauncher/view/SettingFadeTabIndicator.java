...
public class SettingFadeTabIndicator extends LinearLayout {
	...
	public SettingFadeTabIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
		mContext = context;
	}
	...
	private void init() {
		...
		imageSize = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, imageSize, getResources()
						.getDisplayMetrics());
	}
	...
	public void notifyDataSetChanged() {
		...
		for (int i = 0; i < tabCount; i++) {
			addTab(i, mViewPager.getAdapter().getPageTitle(i).toString(),
					tabs.getTabNormalIconResId(i),
					tabs.getTabSelectIconResId(i),
					tabs.getTabNormalTextColorResId(i),
					tabs.getTabSelectTextColorResId(i));
		}
	}

	private void addTab(final int position, String text, int normalResId,
			int selectResId, int textNormalColorResId, int textSelectColorResId) {
		TabView tabView = new TabView(getContext(), text, normalResId,
				selectResId, textNormalColorResId, textSelectColorResId);
	...
	private class FadingImageView extends FrameLayout {
		...
		public FadingImageView(Context context, int normalResId, int selectResId) {
			...
			int imageSize = (int) getResources().getDimension(
					R.dimen.setting_tab_image);
			// Toast.makeText(mContext, "" + textSize,
			// Toast.LENGTH_SHORT).show();
			addView(mNormalImage, 0, new FrameLayout.LayoutParams(imageSize,
					imageSize, Gravity.CENTER));
			addView(mSelectImage, 1, new FrameLayout.LayoutParams(imageSize,
					imageSize, Gravity.CENTER));
		}
	...
	private class FadingTextView extends FrameLayout {
		...
		@TargetApi(Build.VERSION_CODES.HONEYCOMB)
		public FadingTextView(Context context, String text, int normalResId,
				int selectResId) {
			super(context);
	...
	private class TabView extends RelativeLayout {
		...
		public TabView(Context context, String text, int normalResId,
				int selectResId, int textNormalColorResId,
				int textSelectColorResId) {
			super(context);
			RelativeLayout.LayoutParams ivLayoutParams = new RelativeLayout.LayoutParams(
					LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
			fadingImageView = new FadingImageView(context, normalResId,
					selectResId);
			...
			fadingTextView = new FadingTextView(context, text,
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrolled(int position, float positionOffset,
			...
			((TabView) getChildAt(position)).getFadingImageView().setAlpha(
					1.0f - positionOffset);
			((TabView) getChildAt(position)).getFadingTextView().setAlpha(
					1.0f - positionOffset);
			if (position + 1 <= getChildCount() - 1) {
				((TabView) getChildAt(position + 1)).getFadingImageView()
		...
		public void onPageSelected(int position) {
			tabSelect(position);
			notifyDataSetChanged(); // ZJ:点击Tab时需要更新
		}
	}
