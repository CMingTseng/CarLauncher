...
import com.tchip.carlauncher.model.MusicIOnSlidingHandleViewClickListener;

public class MusicMySlidingDrawer extends SlidingDrawer {
	private int mHandleId = 0; // 抽屉行为控件ID
	private int[] mTouchableIds = null; // Handle 部分其他控件ID

	private MusicIOnSlidingHandleViewClickListener mTouchViewClickListener;
	...
	public void setOnSliderHandleViewClickListener(
			MusicIOnSlidingHandleViewClickListener listener) {
		mTouchViewClickListener = listener;
	}

	public MusicMySlidingDrawer(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public MusicMySlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
