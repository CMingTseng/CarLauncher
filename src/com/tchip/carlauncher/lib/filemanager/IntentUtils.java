...
public class IntentUtils {
	...
	public static void createShortcut(Context context, File file) {
		...
		if (file.isDirectory()) {
			...
			shortcutIntent.putExtra(FolderActivity.EXTRA_DIR,
		...
		addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON,
				FileUtils.createFileIcon(file, context, true));
		addIntent.setAction("com.android.launcher.action.INSTALL_SHORTCUT");
		context.sendBroadcast(addIntent);
	}

	public static List<ResolveInfo> getAppsThatHandleFile(File file,
			Context context) {
		return getAppsThatHandleIntent(createFileOpenIntent(file), context);
	}

	public static List<ResolveInfo> getAppsThatHandleIntent(Intent intent,
			Context context) {
