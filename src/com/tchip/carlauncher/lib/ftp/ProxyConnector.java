...
public class ProxyConnector extends Thread {
	...
	public void run() {
		...
		try {
			...
			for (String candidateHostname : candidateProxies) {
				...
				commandSocket = newAuthedSocket(hostname,
						Defaults.REMOTE_PROXY_PORT);
				if (commandSocket == null) {
					continue;
				}
				commandSocket.setSoTimeout(0); // 0 == forever
				// commandSocket.setKeepAlive(true);
				// Now that we have authenticated, we want to start the command
				// session so we can
			...
			while (true) {
				...
				if (numBytes > 0) {
					...
					if (incomingJson.has("action")) {
						// If the incoming JSON object has an "action" field,
						// then it is a
						// request, and not a response
						incomingCommand(incomingJson);
					} else {
						// If the incoming JSON object does not have an "action"
						// field, then
						// it is a response to a request we sent earlier.
						// If there's an object waiting for a response, then
						// that object
						...
						if (responseWaiter != null) {
							...
							myLog.l(Log.INFO,
									"Response received but no responseWaiter");
						}
					}
				} else if (numBytes == 0) {
	...
	// This function is used to spawn a new Thread that will make a request over
	// the
	...
	// The short story is, if the main ProxyConnector command session thread
	// wants to
	...
	/**
	 * Since we want devices to generally stick with the same proxy server, and
	 * we may want to explicitly redirect some devices to other servers, we have
	 * this mechanism to store a "preferred server" on the device.
	 */
	...
	private String[] getProxyList() {
		...
		if (Defaults.release) {
			allProxies = new String[] { "c1.swiftp.org", "c2.swiftp.org",
					"c3.swiftp.org", "c4.swiftp.org", "c5.swiftp.org",
					"c6.swiftp.org", "c7.swiftp.org", "c8.swiftp.org",
					"c9.swiftp.org" };
		} else {
	...
	private boolean checkAndPrintJsonError(JSONObject json)
	...
	/**
	 * Reads our persistent storage, looking for a stored proxy authentication
	 * secret.
	 * 
	 ...
	 * Defaults.getSettingsMode()); return settings.getString("proxySecret",
	 * null); }
	 */
	...
	// Obsolete, there's no authentication anymore
	/*
	 * private void removeSecret() { SharedPreferences settings =
	 * Globals.getContext(). getSharedPreferences(Defaults.getSettingsName(),
	 * Defaults.getSettingsMode()); Editor editor = settings.edit();
	 * editor.remove("proxySecret"); editor.commit(); }
	 */
	...
	private void incomingCommand(JSONObject json) {
		try {
			...
			if (action.equals("control_connection_waiting")) {
				...
				String host = json.getString("host"); // throws JSONException,
														// fine
				preferServer(host);
			...
			// If we're starting a control session register with
			// ftpServerService
		} catch (JSONException e) {
	...
	private Socket newAuthedSocket(String hostname, int port) {
		...
		try {
			...
			byte[] bytes = new byte[IN_BUF_SIZE];
			// Here we assume that the server's response will all be contained
			// in
			// a single read, which may be unsafe for large responses
			numBytes = in.read(bytes);
			if (numBytes == -1) {
				myLog.l(Log.INFO,
						"Proxy socket closed while waiting for auth response");
				return null;
			} else if (numBytes == 0) {
				myLog.l(Log.INFO,
						"Short network read waiting for auth, quitting");
				return null;
			}
	...
	public void quit() {
		setProxyState(State.DISCONNECTED);
		try {
			sendRequest(commandSocket, makeJsonRequest("finished")); // ignore
																		// reply
		...
		try {
			...
			synchronized (this) {
				if (responseWaiter == null) {
					...
					// This code should never run. It is meant to recover from a
					// situation
					// where there is a thread that sent a proxy request but
					// died before
					// starting the subsequent request. If this is the case, the
					// correct
					// behavior is to run the next queued thread in the queue,
					// or if the
					...
					if (queuedRequestThreads.size() == 0) {
						...
						queuedRequestThreads.remove().interrupt(); // start
																	// queued
																	// thread
						queued = true;
					}
			...
			// If a different thread has sent a request and is waiting for a
			// response,
			// then the current thread will be in a queue waiting for an
			// interrupt
			if (queued) {
				// The current thread must wait until we are popped off the
				// waiting queue
			...
			// We have been popped from the wait queue if necessary, and now
			// it's time
			// to send the request.
			try {
				responseWaiter = Thread.currentThread();
				...
				boolean interrupted = false;
				try {
					// Wait for the main ProxyConnector thread to interrupt us,
					// meaning
				...
				// At this point, the main ProxyConnector thread will have
				// stored
				// our response in "JSONObject response".
				myLog.d("Cmd session response was: " + response);
				return response;
			} finally {
				// Make sure that when this request finishes, the next thread on
				// the
				// queue gets started.
				synchronized (this) {
					if (queuedRequestThreads.size() != 0) {
	...
	public JSONObject sendRequest(InputStream in, OutputStream out,
		...
		try {
			if (socket == null) {
				...
				return sendRequest(socket.getInputStream(),
						socket.getOutputStream(), request);
			}
		} catch (IOException e) {
	...
	public ProxyDataSocketInfo pasvListen() {
		try {
			...
			Socket socket = newAuthedSocket(this.hostname,
					Defaults.REMOTE_PROXY_PORT);
			if (socket == null) {
	...
	public Socket dataPortConnect(InetAddress clientAddr, int clientPort) {
		 ...
		 * data_port_connect request to the proxy, containing the IP and port of
		 * the FTP client to which a connection should be made.
		 */
		try {
	...
	/**
	 * Given a socket returned from pasvListen(), send a data_pasv_accept
	 * request over the socket to the proxy, which should result in a socket
	 * that is ready for data transfer with the FTP client. Of course, this will
	 * only work if the FTP client connects to the proxy like it's supposed to.
	 * The client will have already been told to connect by the response to its
	 * PASV command.
	 * 
	 * This should only be called from the onTransfer method of
	 ...
	 * myLog.d("Would return cached quota stats but none retrieved"); } } // If
	 * there's no cached quota stats, or if the called wants fresh stats, //
	 * make a JSON request to the proxy, assuming the command session is open.
	 * try { JSONObject response =
	 * sendCmdSocketRequest(makeJsonRequest("check_quota")); int used, quota;
	 * if(response == null) { myLog.w("check_quota got null response"); return
	 * null; } used = response.getInt("used"); quota = response.getInt("quota");
	 * myLog.d("Got quota response of " + used + "/" + quota); cachedQuotaStats
	 * = new QuotaStats(used, quota) ; return cachedQuotaStats; } catch
	 * (JSONException e) { myLog.w("JSONException in getQuota: " + e); return
	 * null; } }
	 */
	...
	void incrementProxyUsage(long num) {
		...
		proxyUsage += num;
		if (proxyUsage % UPDATE_USAGE_BYTES < oldProxyUsage
				% UPDATE_USAGE_BYTES) {
