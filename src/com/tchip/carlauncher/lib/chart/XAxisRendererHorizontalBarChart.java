...
import com.tchip.carlauncher.lib.chart.BarChart;
import com.tchip.carlauncher.lib.chart.BarData;
import com.tchip.carlauncher.lib.chart.LimitLine;
...
public class XAxisRendererHorizontalBarChart extends XAxisRendererBarChart {
	...
	public XAxisRendererHorizontalBarChart(ViewPortHandler viewPortHandler,
			XAxis xAxis, Transformer trans, BarChart chart) {
	...
	public void computeAxis(float xValAverageLength, List<String> xValues) {
		...
		mXAxis.mLabelWidth = (int) (Utils.calcTextWidth(mAxisLabelPaint,
				longest) + mXAxis.getXOffset() * 3.5f);
		...
		for (int i = mMinX; i <= mMaxX; i += mXAxis.mAxisLabelModulus) {
			...
			if (mViewPortHandler.isInBoundsY(position[1])) {
				...
				c.drawLine(mViewPortHandler.contentLeft(), position[1],
						mViewPortHandler.contentRight(), position[1],
						mGridPaint);
			}
		}
	}
	...
	/**
	 * Draws the LimitLines associated with this axis to the screen. This is the
	 * standard YAxis renderer using the XAxis limit lines.
	 * 
	 * @param c
	 */
	@Override
	public void renderLimitLines(Canvas c) {
		...
		for (int i = 0; i < limitLines.size(); i++) {
			...
			if (label != null && !label.equals("")) {
				...
				float yOffset = l.getLineWidth()
						+ Utils.calcTextHeight(mLimitLinePaint, label) / 2f;
				...
				if (l.getLabelPosition() == LimitLine.LimitLabelPosition.POS_RIGHT) {
					...
					c.drawText(label,
							mViewPortHandler.contentRight() - xOffset, pts[1]
									- yOffset, mLimitLinePaint);

				} else {
					mLimitLinePaint.setTextAlign(Align.LEFT);
					c.drawText(label, mViewPortHandler.offsetLeft() + xOffset,
