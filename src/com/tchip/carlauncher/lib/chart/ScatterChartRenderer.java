...
import com.tchip.carlauncher.lib.chart.ScatterBuffer;
import com.tchip.carlauncher.lib.chart.ScatterData;
import com.tchip.carlauncher.lib.chart.ScatterDataProvider;
import com.tchip.carlauncher.lib.chart.ScatterDataSet;
...
import com.tchip.carlauncher.lib.chart.ScatterChart.ScatterShape;
...
public class ScatterChartRenderer extends DataRenderer {
	...
	public ScatterChartRenderer(ScatterDataProvider chart,
	...
	protected void drawDataSet(Canvas c, ScatterDataSet dataSet) {
		...
		ScatterBuffer buffer = mScatterBuffers[mChart.getScatterData()
		...
		case SQUARE:
			...
			for (int i = 0; i < buffer.size(); i += 2) {
				...
				c.drawRect(buffer.buffer[i] - shapeHalf, buffer.buffer[i + 1]
						- shapeHalf, buffer.buffer[i] + shapeHalf,
						buffer.buffer[i + 1] + shapeHalf, mRenderPaint);
			}
			break;
		case CIRCLE:
		...
		case TRIANGLE:
			...
			for (int i = 0; i < buffer.size(); i += 2) {
				...
				tri.lineTo(buffer.buffer[i] + shapeHalf, buffer.buffer[i + 1]
						+ shapeHalf);
				tri.lineTo(buffer.buffer[i] - shapeHalf, buffer.buffer[i + 1]
						+ shapeHalf);
				tri.close();
		...
		case CROSS:
			...
			for (int i = 0; i < buffer.size(); i += 2) {
				...
				c.drawLine(buffer.buffer[i] - shapeHalf, buffer.buffer[i + 1],
						buffer.buffer[i] + shapeHalf, buffer.buffer[i + 1],
						mRenderPaint);
				c.drawLine(buffer.buffer[i], buffer.buffer[i + 1] - shapeHalf,
						buffer.buffer[i], buffer.buffer[i + 1] + shapeHalf,
						mRenderPaint);
			}
			break;
		default:
	...
	public void drawValues(Canvas c) {
		...
		// if values are drawn
		if (mChart.getScatterData().getYValCount() < mChart
				.getMaxVisibleCount() * mViewPortHandler.getScaleX()) {

			List<ScatterDataSet> dataSets = mChart.getScatterData()
					.getDataSets();
			...
			for (int i = 0; i < mChart.getScatterData().getDataSetCount(); i++) {
				...
				float[] positions = mChart.getTransformer(
				...
				for (int j = 0; j < positions.length * mAnimator.getPhaseX(); j += 2) {
					...
					if ((!mViewPortHandler.isInBoundsLeft(positions[j]) || !mViewPortHandler
							.isInBoundsY(positions[j + 1])))
						continue;
					...
					c.drawText(
							...
							positions[j], positions[j + 1] - shapeSize,
							mValuePaint);
				}
			}
		}
	...
	public void drawHighlighted(Canvas c, Highlight[] indices) {
		...
		for (int i = 0; i < indices.length; i++) {
			...
			ScatterDataSet set = mChart.getScatterData().getDataSetByIndex(
			...
			float[] pts = new float[] { xIndex, mChart.getYChartMax(), xIndex,
					mChart.getYChartMin(), 0, y, mChart.getXChartMax(), y };
