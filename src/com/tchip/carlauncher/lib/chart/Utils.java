...
public abstract class Utils {
	...
	public static void init(Context context) {
		...
		if (context == null) {
			...
			Log.e("MPAndroidChart, Utils.init(...)",
					"PROVIDED CONTEXT OBJECT IS NULL");

		} else {
			ViewConfiguration viewConfiguration = ViewConfiguration
					.get(context);
			mMinimumFlingVelocity = viewConfiguration
					.getScaledMinimumFlingVelocity();
			mMaximumFlingVelocity = viewConfiguration
	 ...
	 * yourself.
	 */
	private static final int POW_10[] = { 1, 10, 100, 1000, 10000, 100000,
	...
	public static float roundToNextSignificant(double number) {
		final float d = (float) Math.ceil((float) Math
				.log10(number < 0 ? -number : number));
