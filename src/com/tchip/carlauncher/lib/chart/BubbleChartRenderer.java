...
import com.tchip.carlauncher.lib.chart.BubbleData;
import com.tchip.carlauncher.lib.chart.BubbleDataProvider;
import com.tchip.carlauncher.lib.chart.BubbleDataSet;
import com.tchip.carlauncher.lib.chart.BubbleEntry;
...
public class BubbleChartRenderer extends DataRenderer {
	...
	public BubbleChartRenderer(BubbleDataProvider chart,
	...
	protected float getShapeSize(float entrySize, float maxSize, float reference) {
		final float factor = (maxSize == 0f) ? 1f : (float) Math.sqrt(entrySize
				/ maxSize);
	...
	protected void drawDataSet(Canvas c, BubbleDataSet dataSet) {
		...
		final float maxBubbleHeight = Math.abs(mViewPortHandler.contentBottom()
		...
		for (int j = minx; j < maxx; j++) {
			...
			pointBuffer[0] = (float) (entry.getXIndex() - minx) * phaseX
					+ (float) minx;
			pointBuffer[1] = (float) (entry.getVal()) * phaseY;
			trans.pointValuesToPixel(pointBuffer);

			float shapeHalf = getShapeSize(entry.getSize(),
					dataSet.getMaxSize(), referenceSize) / 2f;

			if (!mViewPortHandler.isInBoundsTop(pointBuffer[1] + shapeHalf)
					|| !mViewPortHandler.isInBoundsBottom(pointBuffer[1]
							- shapeHalf))
				continue;
			...
			c.drawCircle(pointBuffer[0], pointBuffer[1], shapeHalf,
					mRenderPaint);
		}
	}
	...
	public void drawValues(Canvas c) {
		...
		// if values are drawn
		if (bubbleData.getYValCount() < (int) (Math.ceil((float) (mChart
				.getMaxVisibleCount()) * mViewPortHandler.getScaleX()))) {
			...
			for (BubbleDataSet dataSet : dataSets) {
				...
				valueTextColor = Color.argb(Math.round(255.f * alpha),
						Color.red(valueTextColor), Color.green(valueTextColor),
				...
				final float[] positions = mChart.getTransformer(
						dataSet.getAxisDependency())
						.generateTransformedValuesBubble(entries, phaseX,
								phaseY, minx, maxx);
				...
				for (int j = 0; j < positions.length; j += 2) {
					...
					if ((!mViewPortHandler.isInBoundsLeft(x) || !mViewPortHandler
							.isInBoundsY(y)))
						continue;
					...
					c.drawText(
							...
							x, y + (0.5f * lineHeight), mValuePaint);
				}
			}
		}
	...
	public void drawHighlighted(Canvas c, Highlight[] indices) {
		...
		for (Highlight indice : indices) {
			...
			BubbleDataSet dataSet = bubbleData.getDataSetByIndex(indice
					.getDataSetIndex());
			...
			final BubbleEntry entry = (BubbleEntry) bubbleData
					.getEntryForHighlight(indice);
			...
			final float maxBubbleHeight = Math.abs(mViewPortHandler
					.contentBottom() - mViewPortHandler.contentTop());
			final float referenceSize = Math.min(maxBubbleHeight,
					maxBubbleWidth);
			...
			Color.RGBToHSV(Color.red(originalColor),
					Color.green(originalColor), Color.blue(originalColor),
					_hsvBuffer);
			_hsvBuffer[2] *= 0.5f;
			final int color = Color.HSVToColor(Color.alpha(originalColor),
					_hsvBuffer);
