...
public class Transformer {
	 ...
	 * @param chart
	 */
	public void prepareMatrixValuePx(float xChartMin, float deltaX,
			float deltaY, float yChartMin) {
	...
	/**
	 * Transforms an List of Entry into a float array containing the x and y
	 * values transformed with all matrices for the SCATTERCHART.
	 * 
	 * @param entries
	 * @return
	 */
	public float[] generateTransformedValuesScatter(
			List<? extends Entry> entries, float phaseY) {
	...
	/**
	 * Transforms an List of Entry into a float array containing the x and y
	 * values transformed with all matrices for the BUBBLECHART.
	 * 
	 * @param entries
	 * @return
	 */
	public float[] generateTransformedValuesBubble(
			List<? extends Entry> entries, float phaseX, float phaseY,
			int from, int to) {

		final int count = (int) Math.ceil(to - from) * 2; // (int) Math.ceil((to
															// - from) * phaseX)
															// * 2;
	...
	/**
	 * Transforms an List of Entry into a float array containing the x and y
	 * values transformed with all matrices for the LINECHART.
	 * 
	 * @param entries
	 * @return
	 */
	...
	/**
	 * Transforms an List of Entry into a float array containing the x and y
	 * values transformed with all matrices for the CANDLESTICKCHART.
	 * 
	 * @param entries
	 * @return
	 */
	...
	/**
	 * Transforms an List of Entry into a float array containing the x and y
	 * values transformed with all matrices for the BARCHART.
	 * 
	 * @param entries
	 * @param dataSet
	 *            the dataset index
	...
	public float[] generateTransformedValuesBarChart(
		...
		for (int j = 0; j < valuePoints.length; j += 2) {
			...
			float x = e.getXIndex() + (j / 2 * (setCount - 1)) + dataSet
					+ space * (j / 2) + space / 2f;
			float y = e.getVal();
