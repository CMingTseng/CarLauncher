...
import com.tchip.carlauncher.model.MusicSentenceModel;
...
public class MusicLrcUtil {
	...
	public List<MusicSentenceModel> parseLrc(String line, MusicInfo music) {
		if (line.equals(""))
			return null;
		List<MusicSentenceModel> list = new ArrayList<MusicSentenceModel>();
			...
			if (lastIndex != -1 && index - lastIndex > lastLength + 2) {
				...
				for (String str : temp) {
					...
					if (t != -1) {
						...
						list.add(new MusicSentenceModel(content, t));
					}
				}
				temp.clear();
			}
		...
		Collections.sort(list, new Comparator<MusicSentenceModel>() {

			public int compare(MusicSentenceModel o1, MusicSentenceModel o2) {
		...
		// 结尾为真正第一句歌词的开始
		if (list.size() == 0) {
			list.add(new MusicSentenceModel(music.musicName, 0, Integer.MAX_VALUE));
			return list;
		} else {
			MusicSentenceModel first = list.get(0);
			list.add(0,
					new MusicSentenceModel(music.musicName, 0, first.getFromTime()));
		}
		int size = list.size();
		for (int i = 0; i < size; i++) {
			MusicSentenceModel next = null;
			if (i + 1 < size) {
				next = list.get(i + 1);
			}
			MusicSentenceModel now = list.get(i);
			if (next != null) {
		...
		if (list.size() == 1) {
			...
			MusicSentenceModel last = list.get(list.size() - 1);
