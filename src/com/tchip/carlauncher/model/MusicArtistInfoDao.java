...
public class MusicArtistInfoDao {

	private static final String TABLE_ARTIST = "artist_info";
	private Context mContext;

	public MusicArtistInfoDao(Context context) {
		this.mContext = context;
	}

	public void saveArtistInfo(List<MusicArtistInfo> list) {
		SQLiteDatabase db = MusicDatabaseHelper.getInstance(mContext);
		for (MusicArtistInfo info : list) {
	...
	public List<MusicArtistInfo> getArtistInfo() {
		SQLiteDatabase db = MusicDatabaseHelper.getInstance(mContext);
		...
		while (cursor.moveToNext()) {
			...
			info.number_of_tracks = cursor.getInt(cursor
					.getColumnIndex("number_of_tracks"));
			list.add(info);
		}
		cursor.close();
		return list;
