...
public class ProxyDataSocketFactory extends DataSocketFactory {
	/**
	 * Implements data socket connections that go through our proxy server out
	 * on the net. The proxy sits between the FTP client and us, the server. We
	 * have to build in some coordination between the server and proxy in order
	 * for data sockets to be handled properly.
	 * 
	 * When we receive a "PASV" command from a client, we have to request that
	 * the proxy server open a port, accept a connection, and proxy all data on
	 * that socket between ourself and the FTP client.
	 * 
	 ...
	 * make a connection to the client's IP & port and then proxy all data
	 * between ourself and the FTP client.
	 */
	...
	/**
	 * When the it's time for the SessionThread to actually begin PASV data
	 * transfer with the client, it will call this function to get a valid
	 * socket. The socket will have been created earlier with a call to
	 * onPasv(). The result of calling onTransfer() will be to cause the proxy
	 * to accept the incoming connection from the FTP client and start proxying
	 * back to us (the FTP server). The socket can then be handed back to the
	 * SessionThread which can use it as if it were directly connected to the
	 * client.
	 */
	public Socket onTransfer() {
		...
		if (socket == null) {
			...
			if (proxyConnector == null) {
				myLog.l(Log.INFO,
						"Unexpected null proxyConnector in onTransfer");
				return null;
			}
