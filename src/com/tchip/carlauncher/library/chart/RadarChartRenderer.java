...
import com.tchip.carlauncher.library.chart.RadarData;
import com.tchip.carlauncher.library.chart.RadarDataSet;
...
public class RadarChartRenderer extends DataRenderer {
	...
	protected void drawDataSet(Canvas c, RadarDataSet dataSet) {
		...
		for (int j = 0; j < entries.size(); j++) {
			...
			PointF p = Utils.getPosition(center,
					(e.getVal() - mChart.getYChartMin()) * factor, sliceangle
							* j + mChart.getRotationAngle());
	...
	public void drawValues(Canvas c) {
		...
		for (int i = 0; i < mChart.getData().getDataSetCount(); i++) {
			...
			for (int j = 0; j < entries.size(); j++) {
				...
				PointF p = Utils.getPosition(center,
						(e.getVal() - mChart.getYChartMin()) * factor,
						sliceangle * j + mChart.getRotationAngle());

				c.drawText(
						dataSet.getValueFormatter().getFormattedValue(
								e.getVal()), p.x, p.y - yoffset, mValuePaint);
			}
		}
	}
	...
	protected void drawWeb(Canvas c) {
		...
		for (int j = 0; j < labelCount; j++) {
			...
			for (int i = 0; i < mChart.getData().getXValCount(); i++) {
				...
				float r = (mChart.getYAxis().mEntries[j] - mChart
						.getYChartMin()) * factor;

				PointF p1 = Utils.getPosition(center, r, sliceangle * i
						+ rotationangle);
				PointF p2 = Utils.getPosition(center, r, sliceangle * (i + 1)
						+ rotationangle);
	...
	public void drawHighlighted(Canvas c, Highlight[] indices) {
		...
		for (int i = 0; i < indices.length; i++) {
			...
			RadarDataSet set = mChart.getData().getDataSetByIndex(
			...
			PointF p = Utils.getPosition(center, y * factor, sliceangle * j
					+ mChart.getRotationAngle());

			float[] pts = new float[] { p.x, 0, p.x,
					mViewPortHandler.getChartHeight(), 0, p.y,
