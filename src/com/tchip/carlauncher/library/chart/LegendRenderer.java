	...
	public void renderLegend(Canvas c) {
		...
		case BELOW_CHART_LEFT:
			...
			for (int i = 0, count = labels.length; i < count; i++) {
				...
				if (drawingForm) {
					...
					drawForm(c, posX, posY - mLegend.mTextHeightMax / 2f, i,
							mLegend);
				...
				if (labels[i] != null) {
					...
					if (direction == Legend.LegendDirection.RIGHT_TO_LEFT)
						posX -= Utils.calcTextWidth(mLegendLabelPaint,
								labels[i]);
		...
		case PIECHART_CENTER:
			...
			posY = mViewPortHandler.getChartHeight() / 2f
					- mLegend.mNeededHeight / 2f + mLegend.getYOffset();
			...
			for (int i = 0; i < labels.length; i++) {
				...
				if (labels[i] != null) {
					...
					if (!wasStacked) {
						drawLabel(c, x, posY + mLegend.mTextHeightMax / 2f,
								mLegend.getLabel(i));

						posY += textDrop;
					} else {
						posY += mLegend.mTextHeightMax * 3f;
						drawLabel(c, x, posY - mLegend.mTextHeightMax,
								mLegend.getLabel(i));
					}
