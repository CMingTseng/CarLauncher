...
import com.tchip.carlauncher.library.chart.Legend.LegendPosition;
import com.tchip.carlauncher.library.chart.XAxis;
import com.tchip.carlauncher.library.chart.XAxis.XAxisPosition;
import com.tchip.carlauncher.library.chart.YAxis;
import com.tchip.carlauncher.library.chart.YAxis.AxisDependency;
import com.tchip.carlauncher.library.chart.BarData;
import com.tchip.carlauncher.library.chart.BarLineScatterCandleData;
import com.tchip.carlauncher.library.chart.BarLineScatterCandleDataSet;
import com.tchip.carlauncher.library.chart.DataSet;
...
import com.tchip.carlauncher.library.chart.Approximator;
import com.tchip.carlauncher.library.chart.BarLineScatterCandleDataProvider;
import com.tchip.carlauncher.library.chart.MoveViewJob;
import com.tchip.carlauncher.library.chart.BarLineChartTouchListener;
import com.tchip.carlauncher.library.chart.OnDrawListener;
import com.tchip.carlauncher.library.chart.XAxisRenderer;
import com.tchip.carlauncher.library.chart.YAxisRenderer;
import com.tchip.carlauncher.library.chart.FillFormatter;
import com.tchip.carlauncher.library.chart.Highlight;
import com.tchip.carlauncher.library.chart.PointD;
import com.tchip.carlauncher.library.chart.SelInfo;
...
public abstract class BarLineChartBase<T extends BarLineScatterCandleData<? extends BarLineScatterCandleDataSet<? extends Entry>>>
	...
	protected void init() {
		...
		mAxisRendererRight = new YAxisRenderer(mViewPortHandler, mAxisRight,
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mLogEnabled) {
			...
			Log.i(LOG_TAG, "Drawtime: " + drawtime + " ms, average: " + average
					+ " ms, cycles: " + drawCycles);
		}
	}
	...
	protected void prepareValuePxMatrix() {
		...
		if (mLogEnabled)
			Log.i(LOG_TAG, "Preparing Value-Px Matrix, xmin: " + mXChartMin
					+ ", xmax: " + mXChartMax + ", xdelta: " + mDeltaX);

		mRightAxisTransformer.prepareMatrixValuePx(mXChartMin, mDeltaX,
				mAxisRight.mAxisRange, mAxisRight.mAxisMinimum);
		mLeftAxisTransformer.prepareMatrixValuePx(mXChartMin, mDeltaX,
				mAxisLeft.mAxisRange, mAxisLeft.mAxisMinimum);
	}
	...
	protected void calcMinMax() {
		...
		float leftRange = Math.abs(maxLeft
				- (mAxisLeft.isStartAtZeroEnabled() ? 0 : minLeft));
		float rightRange = Math.abs(maxRight
				- (mAxisRight.isStartAtZeroEnabled() ? 0 : minRight));
		...
		float bottomSpaceRight = rightRange / 100f
				* mAxisRight.getSpaceBottom();
		...
		mAxisLeft.mAxisRange = Math.abs(mAxisLeft.mAxisMaximum
				- mAxisLeft.mAxisMinimum);
		mAxisRight.mAxisRange = Math.abs(mAxisRight.mAxisMaximum
		...
		if (!mCustomViewPortEnabled) {
			...
			if (mAxisRight.needsOffset()) {
				offsetRight += mAxisRight
						.getRequiredWidthSpace(mAxisRendererRight
								.getPaintAxisLabels());
			}
			...
			mViewPortHandler.restrainViewPort(Math.max(min, offsetLeft),
					Math.max(min, offsetTop), Math.max(min, offsetRight),
					Math.max(min, offsetBottom));

			if (mLogEnabled) {
				Log.i(LOG_TAG, "offsetLeft: " + offsetLeft + ", offsetTop: "
						+ offsetTop + ", offsetRight: " + offsetRight
						+ ", offsetBottom: " + offsetBottom);
				Log.i(LOG_TAG, "Content: "
						+ mViewPortHandler.getContentRect().toString());
			}
		}
	...
	protected float[] getMarkerPosition(Entry e, int dataSetIndex) {
		...
		getTransformer(
				mData.getDataSetByIndex(dataSetIndex).getAxisDependency())
	 ...
	 */
	public void zoomIn() {
		Matrix save = mViewPortHandler.zoomIn(getWidth() / 2f,
				-(getHeight() / 2f));
	 ...
	 */
	public void zoomOut() {
		Matrix save = mViewPortHandler.zoomOut(getWidth() / 2f,
				-(getHeight() / 2f));
		mViewPortHandler.refresh(save, this, true);
	}
	...
	public void moveViewToY(float yValue, AxisDependency axis) {
		...
		Runnable job = new MoveViewJob(mViewPortHandler, 0f, yValue
				+ valsInView / 2f, getTransformer(axis), this);
	...
	public void moveViewTo(float xIndex, float yValue, AxisDependency axis) {
		...
		Runnable job = new MoveViewJob(mViewPortHandler, xIndex, yValue
				+ valsInView / 2f, getTransformer(axis), this);
	...
	public void centerViewTo(int xIndex, float yValue, AxisDependency axis) {
		...
		float valsInView = getDeltaY(axis) / mViewPortHandler.getScaleY();
		float xsInView = getXAxis().getValues().size()
	...
	public Highlight getHighlightByTouchPoint(float x, float y) {
		...
		float leftdist = Utils.getMinimumDistance(valsAtIndex, y,
				AxisDependency.LEFT);
		float rightdist = Utils.getMinimumDistance(valsAtIndex, y,
		...
		AxisDependency axis = leftdist < rightdist ? AxisDependency.LEFT
				: AxisDependency.RIGHT;
	 ...
	 * @return
	 */
	public BarLineScatterCandleDataSet<? extends Entry> getDataSetByTouchPoint(
			float x, float y) {
		Highlight h = getHighlightByTouchPoint(x, y);
		if (h != null) {
	...
	public int getLowestVisibleXIndex() {
		float[] pts = new float[] { mViewPortHandler.contentLeft(),
	...
	public int getHighestVisibleXIndex() {
		float[] pts = new float[] { mViewPortHandler.contentRight(),
				mViewPortHandler.contentBottom() };
		getTransformer(AxisDependency.LEFT).pixelsToValue(pts);
		return (pts[0] >= mData.getXValCount()) ? mData.getXValCount() - 1
				: (int) pts[0];
	}
	...
	protected class DefaultFillFormatter implements FillFormatter {
		...
		public float getFillLinePosition(LineDataSet dataSet, LineData data,
			...
			if (dataSet.getYMax() > 0 && dataSet.getYMin() < 0) {
				...
				if (!getAxis(dataSet.getAxisDependency())
