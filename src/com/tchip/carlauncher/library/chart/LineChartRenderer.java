...
import com.tchip.carlauncher.library.chart.ChartAnimator;
import com.tchip.carlauncher.library.chart.CircleBuffer;
import com.tchip.carlauncher.library.chart.LineBuffer;
...
import com.tchip.carlauncher.library.chart.LineDataProvider;
...
public class LineChartRenderer extends DataRenderer {
	...
	public void drawData(Canvas c) {
		...
		if (mDrawBitmap == null || (mDrawBitmap.getWidth() != width)
	...
	protected void drawCubic(Canvas c, LineDataSet dataSet, List<Entry> entries) {
		...
		int maxx = Math.min(dataSet.getEntryPosition(entryTo) + 1,
				entries.size());
		...
		if (size - minx >= 2) {
			...
			// the first cubic
			cubicPath.cubicTo(prev.getXIndex() + prevDx,
					(prev.getVal() + prevDy) * phaseY, cur.getXIndex() - curDx,
					(cur.getVal() - curDy) * phaseY, cur.getXIndex(),
					cur.getVal() * phaseY);
			...
			for (int j = minx + 1, count = Math.min(size, entries.size() - 1); j < count; j++) {
				...
				cubicPath.cubicTo(prev.getXIndex() + prevDx,
						(prev.getVal() + prevDy) * phaseY, cur.getXIndex()
								- curDx, (cur.getVal() - curDy) * phaseY,
			...
			if (size > entries.size() - 1) {
				...
				prevPrev = entries
						.get((entries.size() >= 3) ? entries.size() - 3
								: entries.size() - 2);
	...
	protected void drawCubicFill(Canvas c, LineDataSet dataSet, Path spline,
			Transformer trans, int from, int to) {

		float fillMin = mChart.getFillFormatter().getFillLinePosition(dataSet,
				mChart.getLineData(), mChart.getYChartMax(),
				mChart.getYChartMin());
	...
	protected void drawLinearFill(Canvas c, LineDataSet dataSet,
			List<Entry> entries, int minx, int maxx, Transformer trans) {
		...
		Path filled = generateFilledPath(
				entries,
				mChart.getFillFormatter().getFillLinePosition(dataSet,
						mChart.getLineData(), mChart.getYChartMax(),
						mChart.getYChartMin()), minx, maxx);
	 ...
	 * @return
	 */
	private Path generateFilledPath(List<Entry> entries, float fillMin,
			int from, int to) {
		...
		filled.lineTo(entries.get(from).getXIndex(), entries.get(from).getVal()
				* phaseY);

		// create a new path
		for (int x = from + 1, count = (int) Math.ceil((to - from) * phaseX
				+ from); x < count; x++) {
		...
		filled.lineTo(
				entries.get(
						Math.max(
								Math.min(
										(int) Math.ceil((to - from) * phaseX
												+ from) - 1, entries.size() - 1),
								0)).getXIndex(), fillMin);
	...
	public void drawValues(Canvas c) {
		...
		if (mChart.getLineData().getYValCount() < mChart.getMaxVisibleCount()
			...
			for (int i = 0; i < dataSets.size(); i++) {
				...
				Transformer trans = mChart.getTransformer(dataSet
						.getAxisDependency());
				...
				float[] positions = trans.generateTransformedValuesLine(
						entries, mAnimator.getPhaseX(), mAnimator.getPhaseY(),
						minx, maxx);
				...
				for (int j = 0; j < positions.length; j += 2) {
					...
					c.drawText(
							dataSet.getValueFormatter().getFormattedValue(val),
							x, y - valOffset, mValuePaint);
				}
			}
		}
	}
	...
	protected void drawCircles(Canvas c) {
		...
		for (int i = 0; i < dataSets.size(); i++) {
			...
			Entry entryFrom = dataSet
					.getEntryForXIndex((mMinX < 0) ? 0 : mMinX);
			Entry entryTo = dataSet.getEntryForXIndex(mMaxX);
			...
			for (int j = 0, count = (int) Math.ceil((maxx - minx) * phaseX
					+ minx) * 2; j < count; j += 2) {
	...
	public void drawHighlighted(Canvas c, Highlight[] indices) {
		...
		for (int i = 0; i < indices.length; i++) {
			...
			LineDataSet set = mChart.getLineData().getDataSetByIndex(
			...
			float[] pts = new float[] { xIndex, mChart.getYChartMax(), xIndex,
					mChart.getYChartMin(), mChart.getXChartMin(), y,
					mChart.getXChartMax(), y };

			mChart.getTransformer(set.getAxisDependency()).pointValuesToPixel(
					pts);
