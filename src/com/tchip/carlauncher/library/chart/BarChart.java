...
import com.tchip.carlauncher.library.chart.BarChartRenderer;
import com.tchip.carlauncher.library.chart.XAxisRendererBarChart;
import com.tchip.carlauncher.library.chart.Highlight;

public class BarChart extends BarLineChartBase<BarData> implements
		BarDataProvider {
	...
	protected void init() {
		...
		mXAxisRenderer = new XAxisRendererBarChart(mViewPortHandler, mXAxis,
	...
	protected Highlight getHighlight(double xPosition, double yPosition) {
		...
		if (!mData.isGrouped()) {
			...
			int steps = (int) ((float) xPosition / ((float) setCount + mData
					.getGroupSpace()));
			...
			if (mLogEnabled)
				Log.i(LOG_TAG, "base: " + xPosition + ", steps: " + steps
						+ ", groupSpaceSum: " + groupSpaceSum
			...
			if (mLogEnabled)
				Log.i(LOG_TAG, "xIndex: " + xIndex + ", dataSet: "
						+ dataSetIndex);
	 ...
	 * @return
	 */
	protected Highlight getStackedHighlight(int xIndex, int dataSet,
			double yValue) {

		BarEntry entry = mData.getDataSetByIndex(dataSet).getEntryForXIndex(
				xIndex);
	...
	public int getHighestVisibleXIndex() {
		...
		getTransformer(AxisDependency.LEFT).pixelsToValue(pts);
		return (int) ((pts[0] >= getXChartMax()) ? getXChartMax() / div
				: (pts[0] / div));
	}
}
