...
import com.tchip.carlauncher.library.chart.PieData;
import com.tchip.carlauncher.library.chart.PieDataSet;
...
public class PieChartRenderer extends DataRenderer {
	...
	protected void drawDataSet(Canvas c, PieDataSet dataSet) {
		...
		for (int j = 0; j < entries.size(); j++) {
			...
			if ((Math.abs(e.getVal()) > 0.000001)) {
				...
				if (!mChart.needsHighlight(e.getXIndex(), mChart.getData()
					...
					mBitmapCanvas
							.drawArc(
									...
									(angle + sliceSpace / 2f)
											* mAnimator.getPhaseY(),
									(newangle - sliceSpace / 2f)
											* mAnimator.getPhaseY(), true,
									mRenderPaint);
				}
			}
	...
	public void drawValues(Canvas c) {
		...
		for (int i = 0; i < dataSets.size(); i++) {
			...
			for (int j = 0, maxEntry = Math.min(
					(int) Math.ceil(entries.size() * mAnimator.getPhaseX()),
					entries.size()); j < maxEntry; j++) {
				...
				float x = (float) (r
						* Math.cos(Math.toRadians((rotationAngle
								+ absoluteAngles[cnt] - offset)
				...
				float value = mChart.isUsePercentValuesEnabled() ? entries.get(
						j).getVal()
						/ mChart.getYValueSum() * 100f : entries.get(j)
						.getVal();

				String val = dataSet.getValueFormatter().getFormattedValue(
						value);
	...
	protected void drawHole(Canvas c) {
		...
		if (mChart.isDrawHoleEnabled()) {
			...
			if (transparentCircleRadius > holeRadius
				...
				mBitmapCanvas.drawCircle(center.x, center.y, radius / 100
	...
	protected void drawCenterText(Canvas c) {
		...
		if (mChart.isDrawCenterTextEnabled() && centerText != null) {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				c.drawText(line, center.x, y + maxlineheight * cnt
						- totalheight / 2f, mCenterTextPaint);
				cnt--;
				y -= linespacing;
			}
		}
	...
	public void drawHighlighted(Canvas c, Highlight[] indices) {
		...
		for (int i = 0; i < indices.length; i++) {
			...
			PieDataSet set = mChart.getData().getDataSetByIndex(
			 ...
			 */
			RectF highlighted = new RectF(circleBox.left - shift, circleBox.top
					- shift, circleBox.right + shift, circleBox.bottom + shift);
			...
			mBitmapCanvas.drawArc(highlighted,
					...
					sliceDegrees * mAnimator.getPhaseY() - set.getSliceSpace()
							/ 2f, true, mRenderPaint);
		}
	}
