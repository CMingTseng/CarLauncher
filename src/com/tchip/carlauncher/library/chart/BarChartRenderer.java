...
public class BarChartRenderer extends DataRenderer {
	...
	public void initBuffers() {
		...
		for (int i = 0; i < mBarBuffers.length; i++) {
			...
			mBarBuffers[i] = new BarBuffer(set.getValueCount() * 4
	...
	public void drawValues(Canvas c) {
		...
		if (passesCheck()) {
			...
			for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
				...
				// calculate the correct offset depending on the draw position
				// of
				// the value
				float valueTextHeight = Utils.calcTextHeight(mValuePaint, "8");
				posOffset = (drawValueAboveBar ? -valueOffsetPlus
						: valueTextHeight + valueOffsetPlus);
				negOffset = (drawValueAboveBar ? valueTextHeight
				...
				if (!mChart.isDrawValuesForWholeStackEnabled()) {
					...
					for (int j = 0; j < valuePoints.length
						...
						if (!mViewPortHandler.isInBoundsY(valuePoints[j + 1])
								|| !mViewPortHandler
										.isInBoundsLeft(valuePoints[j]))
							continue;

						float val = entries.get(j / 2).getVal();

						drawValue(c, formatter.getFormattedValue(val),
					...
					for (int j = 0; j < (valuePoints.length - 1)
						...
						if (vals == null) {
							...
							if (!mViewPortHandler
									.isInBoundsRight(valuePoints[j]))
								break;

							if (!mViewPortHandler
									.isInBoundsY(valuePoints[j + 1])
									|| !mViewPortHandler
											.isInBoundsLeft(valuePoints[j]))
								continue;

							drawValue(c,
									formatter.getFormattedValue(e.getVal()),
									valuePoints[j],
									valuePoints[j + 1]
	...
	public void drawHighlighted(Canvas c, Highlight[] indices) {
		...
		for (int i = 0; i < indices.length; i++) {
			...
			BarDataSet set = mChart.getBarData()
					.getDataSetByIndex(dataSetIndex);
			...
			// check outofbounds
			if (index < mChart.getBarData().getYValCount()
					&& index >= 0
					&& index < (mChart.getXChartMax() * mAnimator.getPhaseX())
							/ setCount) {

				BarEntry e = mChart.getBarData()
						.getDataSetByIndex(dataSetIndex)
