...
import com.tchip.carlauncher.library.chart.BarDataSet;
import com.tchip.carlauncher.library.chart.BarEntry;
import com.tchip.carlauncher.library.chart.Entry;
import com.tchip.carlauncher.library.chart.HorizontalBarChartRenderer;
import com.tchip.carlauncher.library.chart.XAxisRendererHorizontalBarChart;
import com.tchip.carlauncher.library.chart.YAxisRendererHorizontalBarChart;
import com.tchip.carlauncher.library.chart.Highlight;
import com.tchip.carlauncher.library.chart.TransformerHorizontalBarChart;
...
public class HorizontalBarChart extends BarChart {
	...
	protected void init() {
		...
		mLeftAxisTransformer = new TransformerHorizontalBarChart(
				mViewPortHandler);
		mRightAxisTransformer = new TransformerHorizontalBarChart(
				mViewPortHandler);

		mRenderer = new HorizontalBarChartRenderer(this, mAnimator,
				mViewPortHandler);
		mAxisRendererLeft = new YAxisRendererHorizontalBarChart(
				mViewPortHandler, mAxisLeft, mLeftAxisTransformer);
		mAxisRendererRight = new YAxisRendererHorizontalBarChart(
				mViewPortHandler, mAxisRight, mRightAxisTransformer);
		mXAxisRenderer = new XAxisRendererHorizontalBarChart(mViewPortHandler,
	...
	protected void calculateOffsets() {
		...
		if (mAxisLeft.needsOffset()) {
			offsetTop += mAxisLeft.getRequiredHeightSpace(mAxisRendererLeft
					.getPaintAxisLabels());
		}

		if (mAxisRight.needsOffset()) {
			offsetBottom += mAxisRight
					.getRequiredHeightSpace(mAxisRendererRight
							.getPaintAxisLabels());
		}
	...
	protected void prepareValuePxMatrix() {
		mRightAxisTransformer.prepareMatrixValuePx(mAxisRight.mAxisMinimum,
				mAxisRight.mAxisRange, mDeltaX, mXChartMin);
		mLeftAxisTransformer.prepareMatrixValuePx(mAxisLeft.mAxisMinimum,
				mAxisLeft.mAxisRange, mDeltaX, mXChartMin);
	}
	...
	public int getHighestVisibleXIndex() {
		...
		getTransformer(AxisDependency.LEFT).pixelsToValue(pts);
		return (int) ((pts[1] >= getXChartMax()) ? getXChartMax() / div
				: (pts[1] / div));
	}
}
