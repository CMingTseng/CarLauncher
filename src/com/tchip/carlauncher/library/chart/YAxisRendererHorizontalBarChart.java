...
import com.tchip.carlauncher.library.chart.LimitLine;
import com.tchip.carlauncher.library.chart.LimitLine.LimitLabelPosition;
import com.tchip.carlauncher.library.chart.YAxis;
import com.tchip.carlauncher.library.chart.YAxis.AxisDependency;
import com.tchip.carlauncher.library.chart.YAxis.YAxisLabelPosition;
...
public class YAxisRendererHorizontalBarChart extends YAxisRenderer {
	...
	public void computeAxis(float yMin, float yMax) {
		...
		if (mViewPortHandler.contentHeight() > 10
			...
			PointD p1 = mTrans.getValuesByTouchPoint(
					mViewPortHandler.contentLeft(),
					mViewPortHandler.contentTop());
			PointD p2 = mTrans.getValuesByTouchPoint(
	...
	protected void drawYLabels(Canvas c, float fixedPosition,
		...
		for (int i = 0; i < mYAxis.mEntryCount; i++) {
			...
			c.drawText(text, positions[i * 2], fixedPosition - offset,
					mAxisLabelPaint);
		}
	}
	 ...
	 * standard XAxis renderer using the YAxis limit lines.
	 * 
	 * @param c
	 */
	@Override
	public void renderLimitLines(Canvas c) {
		...
		for (int i = 0; i < limitLines.size(); i++) {
			...
			if (label != null && !label.equals("")) {
				...
				float yOffset = Utils.calcTextHeight(mLimitLinePaint, label)
						+ add / 2f;

				if (l.getLabelPosition() == LimitLabelPosition.POS_RIGHT) {
					c.drawText(label, pts[0] + xOffset,
							mViewPortHandler.contentBottom() - add,
							mLimitLinePaint);
				} else {
					c.drawText(label, pts[0] + xOffset,
							mViewPortHandler.contentTop() + yOffset,
							mLimitLinePaint);
				}
			}
		}
