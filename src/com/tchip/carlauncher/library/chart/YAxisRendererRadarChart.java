...
public class YAxisRendererRadarChart extends YAxisRenderer {
	...
	public YAxisRendererRadarChart(ViewPortHandler viewPortHandler,
			YAxis yAxis, RadarChart chart) {
	...
	public void renderAxisLabels(Canvas c) {
		...
		for (int j = 0; j < labelCount; j++) {
			...
			if (j == labelCount - 1
					&& mYAxis.isDrawTopYLabelEntryEnabled() == false)
				break;
	...
	public void renderLimitLines(Canvas c) {
		...
		for (int i = 0; i < limitLines.size(); i++) {
			...
			for (int j = 0; j < mChart.getData().getXValCount(); j++) {
				...
				PointF p = Utils.getPosition(center, r,
