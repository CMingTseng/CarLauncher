...
import com.tchip.carlauncher.library.chart.BarBuffer;
import com.tchip.carlauncher.library.chart.HorizontalBarBuffer;
...
import com.tchip.carlauncher.library.chart.BarDataProvider;
import com.tchip.carlauncher.library.chart.Transformer;
import com.tchip.carlauncher.library.chart.Utils;
import com.tchip.carlauncher.library.chart.ValueFormatter;
...
public class HorizontalBarChartRenderer extends BarChartRenderer {
	...
	public HorizontalBarChartRenderer(BarDataProvider chart,
	...
	public void initBuffers() {
		...
		for (int i = 0; i < mBarBuffers.length; i++) {
			...
			mBarBuffers[i] = new HorizontalBarBuffer(set.getValueCount() * 4
					* set.getStackSize(), barData.getGroupSpace(),
		...
		for (int j = 0; j < buffer.size(); j += 4) {
			...
			if (mChart.isDrawBarShadowEnabled()) {
				c.drawRect(mViewPortHandler.contentLeft(),
						buffer.buffer[j + 1], mViewPortHandler.contentRight(),
	...
	public void drawValues(Canvas c) {
		...
		if (passesCheck()) {
			...
			for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
				...
				boolean isInverted = mChart.isInverted(dataSet
						.getAxisDependency());
				...
				if (!mChart.isDrawValuesForWholeStackEnabled()) {
					...
					for (int j = 0; j < valuePoints.length
							* mAnimator.getPhaseX(); j += 2) {
						...
						if (!mViewPortHandler
								.isInBoundsBottom(valuePoints[j + 1]))
							continue;

						float val = entries.get(j / 2).getVal();
						String valueText = formatter.getFormattedValue(val);

						// calculate the correct offset depending on the draw
					...
					for (int j = 0; j < (valuePoints.length - 1)
							* mAnimator.getPhaseX(); j += 2) {
						...
						if (vals == null) {
							...
							// calculate the correct offset depending on the
							// draw position of the value
							float valueTextWidth = Utils.calcTextWidth(
									mValuePaint, valueText);
							posOffset = (drawValueAboveBar ? valueOffsetPlus
									: -(valueTextWidth + valueOffsetPlus));
							negOffset = (drawValueAboveBar ? -(valueTextWidth + valueOffsetPlus)
									: valueOffsetPlus);
	...
	@Override
	protected void prepareBarHighlight(float x, float y, float barspaceHalf,
			float from, Transformer trans) {
	...
	@Override
	public float[] getTransformedValues(Transformer trans,
			List<BarEntry> entries, int dataSetIndex) {
		return trans.generateTransformedValuesHorizontalBarChart(entries,
				dataSetIndex, mChart.getBarData(), mAnimator.getPhaseY());
	}
