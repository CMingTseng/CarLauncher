	...
	private static class EasingFunctions {
		...
		public static final EasingFunction EaseInOutQuart = new EasingFunction() {
			...
			public float getInterpolation(float input) {
				...
				position -= 2.f;
				return -0.5f
						* (position * position * position * position - 2.f);
			}
		};
		...
		public static final EasingFunction EaseInExpo = new EasingFunction() {
			// @Override
			// public float ease(long elapsed, long duration) {
			// return (elapsed == 0) ? 0.f : (float) Math.pow(2.f, 10.f *
			// (elapsed
			// / (float) duration - 1.f));
			// }
			@Override
			public float getInterpolation(float input) {
				return (input == 0) ? 0.f : (float) Math.pow(2.f,
						10.f * (input - 1.f));
			}
		};
		...
		public static final EasingFunction EaseOutExpo = new EasingFunction() {
			// @Override
			// public float ease(long elapsed, long duration) {
			// return (elapsed == duration) ? 1.f : (-(float) Math.pow(2.f,
			// -10.f *
			// elapsed
			// / (float) duration) + 1.f);
			// }
			...
			@Override
			public float getInterpolation(float input) {
				return (input == 1f) ? 1.f : (-(float) Math.pow(2.f, -10.f
						* (input + 1.f)));
			}
		};
		...
		public static final EasingFunction EaseInOutExpo = new EasingFunction() {
			...
			public float getInterpolation(float input) {
				...
				float position = input / 0.5f;
				if (position < 1.f) {
					return 0.5f * (float) Math
							.pow(2.f, 10.f * (position - 1.f));
				}
		...
		public static final EasingFunction EaseInOutCirc = new EasingFunction() {
			...
			public float getInterpolation(float input) {
				...
				if (position < 1.f) {
					return -0.5f
							* ((float) Math.sqrt(1.f - position * position) - 1.f);
				}
				return 0.5f * ((float) Math.sqrt(1.f - (position -= 2.f)
						* position) + 1.f);
			}
				...
				if (position < 1.f) {
					return -.5f
							* ((float) Math.pow(2.f, 10.f * (position -= 1.f)) * (float) Math
									.sin((position * 1f - s) * (2.f * Math.PI)
											/ p));
				}
				return (float) Math.pow(2.f, -10.f * (position -= 1.f))
						* (float) Math.sin((position * 1f - s)
								* (2.f * Math.PI) / p) * .5f + 1.f;
			}
		};
