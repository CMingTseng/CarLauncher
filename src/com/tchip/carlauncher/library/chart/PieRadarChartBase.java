...
import com.tchip.carlauncher.library.chart.Easing;
import com.tchip.carlauncher.library.chart.Legend.LegendPosition;
import com.tchip.carlauncher.library.chart.ChartData;
import com.tchip.carlauncher.library.chart.DataSet;
import com.tchip.carlauncher.library.chart.Entry;
import com.tchip.carlauncher.library.chart.PieRadarChartTouchListener;
...
public abstract class PieRadarChartBase<T extends ChartData<? extends DataSet<? extends Entry>>>
	...
	protected void calculateOffsets() {
		...
		if (mLegend != null && mLegend.isEnabled()) {
			...
			if (mLegend.getPosition() == LegendPosition.RIGHT_OF_CHART_CENTER) {
				...
				float legendHeight = mLegend.mNeededHeight
						+ mLegend.mTextHeightMax;

				PointF c = getCenter();

				PointF bottomRight = new PointF(getWidth() - legendWidth + 15,
						legendHeight + 15);
				float distLegend = distanceToCenter(bottomRight.x,
						bottomRight.y);
				...
				if (bottomRight.y >= c.y
						&& getHeight() - legendWidth > getWidth()) {
				...
				PointF bottomLeft = new PointF(legendWidth - 15,
						legendHeight + 15);
		...
		float offsetBottom = Math.max(min,
				Math.max(getRequiredBaseOffset(), legendBottom));

		mViewPortHandler.restrainViewPort(offsetLeft, offsetTop, offsetRight,
				offsetBottom);
	...
	protected PointF getPosition(PointF center, float dist, float angle) {
		...
		PointF p = new PointF((float) (center.x + dist
				* Math.cos(Math.toRadians(angle))), (float) (center.y + dist
	...
	private float getFullLegendWidth() {
		return mLegend.mTextWidthMax + mLegend.getFormSize()
	...
	public void spin(int durationmillis, float fromangle, float toangle,
		...
		ObjectAnimator spinAnimator = ObjectAnimator.ofFloat(this,
				"rotationAngle", fromangle, toangle);
		spinAnimator.setDuration(durationmillis);
		spinAnimator
				.setInterpolator(Easing.getEasingFunctionFromOption(easing));
