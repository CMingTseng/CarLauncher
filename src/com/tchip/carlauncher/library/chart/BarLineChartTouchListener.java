...
import com.tchip.carlauncher.library.chart.BarLineChartBase;
import com.tchip.carlauncher.library.chart.HorizontalBarChart;
...
public class BarLineChartTouchListener<T extends BarLineChartBase<? extends BarLineScatterCandleData<? extends BarLineScatterCandleDataSet<? extends Entry>>>>
	...
	public boolean onTouch(View v, MotionEvent event) {
			...
			if (mTouchMode == DRAG) {
				...
				if (mChart.isScaleXEnabled() || mChart.isScaleYEnabled())
					...
					&& Math.abs(distance(event.getX(), mTouchStartPoint.x,
							event.getY(), mTouchStartPoint.y)) > 5f) {
		...
		case MotionEvent.ACTION_POINTER_UP:
			Utils.velocityTrackerPointerUpCleanUpIfNecessary(event,
					mVelocityTracker);
	...
	private void saveTouchStart(MotionEvent event) {
		...
		mClosestDataSetToTouch = mChart.getDataSetByTouchPoint(event.getX(),
				event.getY());
	}
	...
	private void performDrag(MotionEvent event) {
		...
		if (mChart.isAnyAxisInverted()
				...
				&& mChart.getAxis(mClosestDataSetToTouch.getAxisDependency())
						.isInverted()) {
	...
	public PointF getTrans(float x, float y) {
		...
		if (mChart.isAnyAxisInverted()
				&& mClosestDataSetToTouch != null
				&& mChart
						.isInverted(mClosestDataSetToTouch.getAxisDependency())) {
	...
	public boolean onDoubleTap(MotionEvent e) {
		...
		if (mChart.isDoubleTapToZoomEnabled()) {
			...
			if (mChart.isLogEnabled())
				Log.i("BarlineChartTouch", "Double-Tap, Zooming In, x: "
						+ trans.x + ", y: " + trans.y);
		}
	...
	public void computeScroll() {
		...
		if (Math.abs(mDecelerationVelocity.x) >= 0.01
				|| Math.abs(mDecelerationVelocity.y) >= 0.01)
			Utils.postInvalidateOnAnimation(mChart); // This causes
														// computeScroll to
