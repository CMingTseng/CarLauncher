...
public abstract class Renderer {
	...
	/**
	 * Returns true if the specified value fits in between the provided min and
	 * max bounds, false if not.
	 * 
	 * @param val
	 * @param min
	 * @param max
	...
	public void calcXBounds(BarLineScatterCandleDataProvider chart,
		...
		mMaxX = Math.min((high / xAxisModulus) * (xAxisModulus) + xAxisModulus,
				(int) chart.getXChartMax());

		// double minx =
		// trans.getValuesByTouchPoint(mViewPortHandler.contentLeft(), 0).x;
		// double maxx =
		// trans.getValuesByTouchPoint(mViewPortHandler.contentRight(), 0).x;
		//
