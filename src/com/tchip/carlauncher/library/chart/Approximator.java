...
public class Approximator {
	...
	private void algorithmDouglasPeucker(List<Entry> entries, double epsilon,
		...
		for (int i = start + 1; i < end; i++) {
			double dist = calcAngleBetweenLines(firstEntry, lastEntry,
					firstEntry, entries.get(i));
	 ...
	 *            line endpoint
	 * @param entryPoint
	 *            the point to which the distance is measured from the line
	 * @return
	 */
	public double calcPointToLineDistance(Entry startEntry, Entry endEntry,
			Entry entryPoint) {

		float xDiffEndStart = (float) endEntry.getXIndex()
				- (float) startEntry.getXIndex();
		float xDiffEntryStart = (float) entryPoint.getXIndex()
				- (float) startEntry.getXIndex();

		double normalLength = Math.sqrt((xDiffEndStart) * (xDiffEndStart)
