...
import com.tchip.carlauncher.library.chart.PieChart;
import com.tchip.carlauncher.library.chart.PieRadarChartBase;
import com.tchip.carlauncher.library.chart.RadarChart;
...
public class PieRadarChartTouchListener extends SimpleOnGestureListener
			...
			case MotionEvent.ACTION_UP:
				...
				if (mChart.isDragDecelerationEnabled()) {
					...
					if (mDecelerationAngularVelocity != 0.f) {
						...
						Utils.postInvalidateOnAnimation(mChart); // This causes
																	// computeScroll
																	// to fire,
																	// recommended
																	// for this
			...
			if (index < 0) {
				...
				if (mChart instanceof RadarChart) {
					...
					dataSetIndex = Utils.getClosestDataSetIndex(valsAtIndex,
							distance / ((RadarChart) mChart).getFactor(), null);
				}
	...
	private void sampleVelocity(float touchLocationX, float touchLocationY) {
		...
		_velocitySamples.add(new AngularVelocitySample(currentTime, mChart
	...
	private float calculateVelocity() {
		...
		AngularVelocitySample lastSample = _velocitySamples
				.get(_velocitySamples.size() - 1);

		// Look for a sample that's closest to the latest sample, but not the
		// same, so we can deduce the direction
		...
		// Calculate clockwise/ccw by choosing two values that should be closest
		// to each other,
		// so if the angles are two far from each other we know they are
		// inverted "for sure"
		...
		// Now if the "gesture" is over a too big of an angle - then we know the
		// angles are inverted, and we need to move them closer to each other
		// from both sides of the 360.0 wrapping point
		...
		// The velocity
		float velocity = Math.abs((lastSample.angle - firstSample.angle)
				/ timeDelta);
	...
	public void computeScroll() {
		...
		mDecelerationAngularVelocity *= mChart
				.getDragDecelerationFrictionCoef();

		final float timeInterval = (float) (currentTime - mDecelerationLastTime) / 1000.f;

		mChart.setRotationAngle(mChart.getRotationAngle()
				+ mDecelerationAngularVelocity * timeInterval);
