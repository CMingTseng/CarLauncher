...
import com.tchip.carlauncher.library.chart.PieChartRenderer;
...
public class PieChart extends PieRadarChartBase<PieData> {
	...
	protected void calculateOffsets() {
		...
		// the pie-chart)
		mCircleBox.set(c.x - boxSize, c.y - boxSize, c.x + boxSize, c.y
				+ boxSize);
	}
		...
		for (int i = 0; i < mData.getDataSetCount(); i++) {
			...
			for (int j = 0; j < entries.size(); j++) {
				...
				if (cnt == 0) {
					...
					mAbsoluteAngles[cnt] = mAbsoluteAngles[cnt - 1]
							+ mDrawAngles[cnt];
				}
	...
	public void setTransparentCircleColor(int color) {
		((PieChartRenderer) mRenderer).getPaintTransparentCircle().setColor(
				color);
	}
