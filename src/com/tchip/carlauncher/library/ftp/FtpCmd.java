...
public abstract class FtpCmd implements Runnable {
	...
	private static FtpCmd getCmd(String cmd, SessionThread sessionThread,
			String input) {
		if ("SYST".equals(cmd))
	...
	/**
	 * An FTP parameter is that part of the input string that occurs after the
	 * first space, including any subsequent spaces. Also, we want to chop off
	 * the trailing '\r\n', if present.
	 * 
	 * Some parameters shouldn't be logged or output (e.g. passwords), so the
	 * caller can use silent==true in that case.
	 */
