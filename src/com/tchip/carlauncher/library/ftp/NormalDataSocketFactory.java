...
public class NormalDataSocketFactory extends DataSocketFactory {
	/**
	 * This class implements normal, traditional opening and closing of data
	 * sockets used for transmitting directory listings and file contents. PORT
	 * and PASV work according to the FTP specs. This is in contrast to a
	...
	private void clearState() {
		/**
		 * Clears the state of this object, as if no pasv() or port() had
		 * occurred. All sockets are closed.
		 */
		if (server != null) {
			try {
				server.close();
	...
	public Socket onTransfer() {
		if (server == null) {
			...
			try {
				...
				myLog.l(Log.INFO, "Couldn't open PORT data socket to: "
	...
	/**
	 * Return the port number that the remote client should be informed of (in
	 * the body of the PASV response).
	 * 
	 * @return The port number, or -1 if error.
	 */
	public int getPortNumber() {
		if (server != null) {
			return server.getLocalPort(); // returns -1 if serversocket is
											// unbound
		} else {
			return -1;
		}
