...
import com.tchip.carlauncher.adapter.MusicLyricAdapter;
import com.tchip.carlauncher.adapter.MusicLyricDownloadManager;
import com.tchip.carlauncher.adapter.MusicLyricLoadHelper;
import com.tchip.carlauncher.adapter.MusicMyAdapter;
import com.tchip.carlauncher.adapter.MusicLyricLoadHelper.LyricListener;
...
import com.tchip.carlauncher.model.MusicLyricSentence;
import com.tchip.carlauncher.model.MusicSPStorage;
import com.tchip.carlauncher.service.MusicServiceManager;
import com.tchip.carlauncher.ui.activity.MusicPlayQueueActivity;
import com.tchip.carlauncher.util.MusicTimer;
import com.tchip.carlauncher.view.MusicMySlidingDrawer;

/**
 * 底部弹出的歌词界面控制
 */
@SuppressLint("HandlerLeak")
public class MusicSlidingDrawerManager implements OnClickListener,
		OnSeekBarChangeListener, Constant, OnDrawerOpenListener,
		OnDrawerCloseListener {

	private MusicMySlidingDrawer mSliding;
	...
	private MusicLyricDownloadManager mLyricDownloadManager;
	private MusicLyricLoadHelper mLyricLoadHelper;
	private MusicLyricAdapter mLyricAdapter;
	...
	public MusicSlidingDrawerManager(Activity a, MusicServiceManager sm, View view) {
		this.mServiceManager = sm;
		this.mActivity = a;
		this.mView = view;
		mFavoriteDao = new MusicFavoriteInfoDao(a);
		mMusicDao = new MusicInfoDao(a);
		mSp = new MusicSPStorage(a);
		mLyricDownloadManager = new MusicLyricDownloadManager(a);
		mLyricLoadHelper = new MusicLyricLoadHelper();
		...
		// 歌词秀设置---------------------------------------------------------------
		mLyricAdapter = new MusicLyricAdapter(a);
		...
		view_in = AnimationUtils.loadAnimation(a, R.anim.music_fade_in);
		view_out = AnimationUtils.loadAnimation(a, R.anim.music_fade_out);
	...
	private void initView() {
		...
		mSliding = (MusicMySlidingDrawer) findViewById(R.id.slidingDrawer);
	...
	private void showLrcDialog() {
		View view = View.inflate(mActivity, R.layout.music_lrc_dialog, null);
	...
	public void loadLyric(MusicInfo playingSong) {
		...
		// 取得歌曲同目录下的歌词文件绝对路径
		String lyricFilePath = MyApplication.lrcPath + "/" + playingSong.musicName
				+ ".lrc";
	...
	private void loadLyricByHand(String musicName, String artist) {
		// 取得歌曲同目录下的歌词文件绝对路径
		String lyricFilePath = MyApplication.lrcPath + "/" + musicName + ".lrc";
		File lyricfile = new File(lyricFilePath);
	...
	private LyricListener mLyricListener = new LyricListener() {
		...
		@Override
		public void onLyricLoaded(List<MusicLyricSentence> lyricSentences, int index) {
	...
	public void setListViewAdapter(MusicMyAdapter adapter) {
		mAdapter = adapter;
	}
