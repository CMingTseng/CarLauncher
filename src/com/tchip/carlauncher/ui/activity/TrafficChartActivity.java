...
public class TrafficChartActivity extends Activity {
	...
	private void initView() {
		...
		if (bundle != null) {
			...
			Toast.makeText(this, getResources().getString(R.string.data_error),
					Toast.LENGTH_SHORT).show();
			finish();
		}

		mChart = (LineChart) findViewById(R.id.chart1);
		mChart.setNoDataTextDescription(getResources().getString(
				R.string.data_not_load));
	}
	...
	private void initChart() {
		...
		TrafficMyMarkerView mv = new TrafficMyMarkerView(this,
				R.layout.trafftic_custom_marker_view);
	...
	private void setData(List<TrafficDateTrafficModel> datas) {
		...
		for (int i = 0; i < datas.size(); i++) {
			xVals.add(new SimpleDateFormat("M.d").format(new Date(datas.get(i)
					.getDate())));
		}
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			yVals.add(new Entry(TrafficUtils
					.dataToMB(datas.get(i).getTraffic()), i));
		}

		// create a dataset and give it a type
		LineDataSet set1 = new LineDataSet(yVals, DATA_SIZE
				+ getResources().getString(R.string.traffic_state));
		...
		Collections.sort(datas, new Comparator<TrafficDateTrafficModel>() {
			@Override
			public int compare(TrafficDateTrafficModel lhs,
					TrafficDateTrafficModel rhs) {
				return (int) (rhs.getTraffic() - lhs.getTraffic());
			}
		});
		leftAxis.setAxisMaxValue(TrafficUtils.dataToMB(datas.get(0)
				.getTraffic()) + 10.0f);
	...
	private class FetchDataTask extends
			AsyncTask<Void, Void, List<TrafficDateTrafficModel>> {
		...
		protected List<TrafficDateTrafficModel> doInBackground(Void... params) {
			return TrafficUtils.fetchDayTrrafic(TrafficChartActivity.this, uid,
					DATA_SIZE);
		}
