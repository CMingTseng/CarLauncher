 ...
 * 歌词的显示控制
 */
public class MusicLyricLoadHelper {
	/** 用于向外通知歌词载入、变化的监听器 */
	public interface LyricListener {
		 ...
		 *            正在播放的句子在句子集合中的索引号
		 */
		public abstract void onLyricLoaded(List<MusicLyricSentence> lyricSentences,
	...
	private static final String TAG = MusicLyricLoadHelper.class.getSimpleName();

	/** 句子集合 */
	private ArrayList<MusicLyricSentence> mLyricSentences = new ArrayList<MusicLyricSentence>();
	...
	public List<MusicLyricSentence> getLyricSentences() {
			...
			if (file.exists()) {
				...
				try {
					...
					Collections.sort(mLyricSentences,
							new Comparator<MusicLyricSentence>() {
								// 内嵌，匿名的compare类
								public int compare(MusicLyricSentence object1,
										MusicLyricSentence object2) {
			...
			if (lastIndex != -1 && index - lastIndex > lastLength + 2) {
				...
				for (String string : times) {
					...
					if (t != -1) {
						...
						mLyricSentences.add(new MusicLyricSentence(t, content));
					}
				}
				times.clear();
			}
