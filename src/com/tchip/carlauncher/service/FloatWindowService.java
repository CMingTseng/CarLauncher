...
public class FloatWindowService extends Service {
	...
	private void createFloatView() {
		...
		// 浮动窗口按钮
		imageRecordingHint = (ImageView) mFloatLayout.findViewById(R.id.imageRecordingHint);
		...
		imageRecordingHint.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				wmParams.x = (int) event.getRawX()
						- imageRecordingHint.getMeasuredWidth() / 2;
				MyLog.i("RawX" + event.getRawX());
				MyLog.i("X" + event.getX());
				// 减25为状态栏的高度
				wmParams.y = (int) event.getRawY()
						- imageRecordingHint.getMeasuredHeight() / 2 - 25;
				MyLog.i("RawY" + event.getRawY());
